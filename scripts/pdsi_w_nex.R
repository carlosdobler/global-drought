...
# CALCULATION OF scPDSI WITH NEX CMIP6 DATA
...
# results directory
dir_res <- str_glue("/mnt/bucket_mine/results/global_drought_ww/pdsi/nex/{model}")
dir.create(dir_res)
...
walk(d %>% year() %>% unique(), function(yr){
  ...
  # create file
  f_name <- 
    str_glue("{dir_res}/nex-{model}_mon_pdsi_{yr}.nc")
